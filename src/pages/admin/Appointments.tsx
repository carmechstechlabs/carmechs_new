...
export function Appointments() {
  ...
  const filteredAppointments = appointments.filter(app => {
    ...
    const matchesMake = makeFilter === "all" || app.make === makeFilter;
    const matchesModel = modelFilter === "all" || app.model === modelFilter;
    const matchesFuel = fuelFilter === "all" || app.fuelType === fuelFilter;
    ...
    return matchesSearch && matchesStatus && matchesService && matchesDate && matchesPriority && matchesPayment && matchesMake && matchesModel && matchesFuel;
  }).sort((a, b) => {
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full lg:w-auto">
          <div className="relative w-full sm:w-80 group">
            ...
            <div className="flex items-center gap-3 bg-white border border-slate-200 rounded-2xl px-4 h-12 shadow-sm">
              ...
              >
                <option value="all" className="bg-white">All Makes</option>
                {carMakes.map(m => (
            ...
            <div className="flex items-center gap-3 bg-white border border-slate-200 rounded-2xl px-4 h-12 shadow-sm">
              ...
              >
                <option value="all" className="bg-white">All Models</option>
                {carModels.filter(m => makeFilter === 'all' || m.makeId === carMakes.find(mk => mk.name === makeFilter)?.id).map(m => (
                  <option key={m.id} value={m.name} className="bg-white">{m.name}</option>
                ))}
              </select>
            </div>
            <div className="flex items-center gap-3 bg-white border border-slate-200 rounded-2xl px-4 h-12 shadow-sm">
              ...
              >
                <option value="all" className="bg-white">All Fuel</option>
                {fuelTypes.map(f => (
            ...
            >
              ...
              <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-8 max-h-[70vh] overflow-y-auto scrollbar-none">
                ...
                <div className="space-y-8">
                  ...
                  {selectedAppointment.issuePhotos && selectedAppointment.issuePhotos.length > 0 && (
                    <div className="space-y-4">
                      <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2">
                        <Activity className="h-3 w-3 text-primary" /> Visual Diagnostics
                      </h4>
                      <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100">
                        <div className="grid grid-cols-2 gap-2">
                          {selectedAppointment.issuePhotos.map((photo, idx) => (
                            <div key={idx} className="aspect-square rounded-xl overflow-hidden border border-slate-200 bg-white">
                              <img 
                                src={photo} 
                                alt={`Issue ${idx + 1}`} 
